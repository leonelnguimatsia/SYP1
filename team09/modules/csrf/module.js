...
function csrf(req, res, target, method) {    
    ...
    res.cookie(token.getTokenName(), token.getCookieToken(), {...options.cookie, expires: token.getExpirationDate()})
...
function validate(req, res, token) {
    ...
    let t = token || globalCSRFCache.get(sid/*, target, method*/)?.[0]
    ...
    // remove cookie
    res.clearCookie(t.getTokenName())
